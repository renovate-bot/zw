...
However, `mv` also can be used, if a proper `atpull`, an ice–modifier ran at **update** of plugin, will be used:
...
There’s also `atinit` ice-modifier, executed before each loading of plugin or snippet.
...
The ice-mod `wait` allows the user to postpone the loading of a plugin to the moment when the processing of `.zshrc` is finished and the first prompt is being shown. It is like Windows – during startup, it shows desktop even though it still loads data in the background.
...
If you're interested in more examples then check out the [playground repository][19] where users have uploaded the `~/.zshrc` and other ZI configurations.
