...
2. Install and run Ruby [gems](https://github.com/rubygems/rubygems) and
   [Node](https://github.com/npm/cli) modules from within a local directory with
   [\$GEM_HOME](https://guides.rubygems.org/command-reference/#gem-environment) and
...
The `sbin''` ice that creates forwarder-scripts instead of forwarder-functions
(created by the `fbin''` ice and elaborated in this `How it works …` section) turned out to be the proper,
...
- `-i/--from-ices` – normally the code looks for the shim files by examining
  ...
  for example, then this means that there has to be such shim already created,
